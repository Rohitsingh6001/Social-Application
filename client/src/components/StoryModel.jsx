...
const StoryModel = ({ setShowModel, fetchStories }) => {
  ...
  return (
    <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blue text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="flex gap-2 mt-4">
            <button onClick={() => {setMode('text'); setMedia(null); setPreviewUrl(null)}} className={`flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode === 'text' ? "bg-white text-black" : "bg-zinc-800"}`}>
              <TextIcon size={19}/>Text
            </button>
            <label className={`flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode ==='media' ? "bg-white text-color" : "bg-zinc-800"}`}>
              <input onChange={(e) =>{handleMediaUpload(e); setMode('media')}} type="file" accept="image/*, vidio/*" className="hidden" />
              <Upload size={18}/> Photo/Vedio
            </label>
        </div>
